...
{
    std::shared_ptr<Sum> sum = std::make_shared<Sum>(Money::dollar(3), Money::dollar(4));
    Bank bank;
...
TEST_F(MoneyTest, testSumPlusMoney)
{
    std::shared_ptr<Expression> fiveDollar = Money::dollar(5);
    std::shared_ptr<Expression> tenFrancs = Money::franc(10);
    Bank bank;
    bank.addRate("CHF", "USD", 2);
    std::shared_ptr<Sum> sum = std::make_shared<Sum>(fiveDollar, tenFrancs);
    auto sumNew = sum->plus(fiveDollar);
    Money result = bank.reduce(*sumNew, "USD");
    EXPECT_EQ(Money::dollar(15)->amount(), result.amount());
}

TEST_F(MoneyTest, testSumMultipleBy)
{
    std::shared_ptr<Expression> fiveDollar = Money::dollar(5);
    std::shared_ptr<Expression> tenFrancs = Money::franc(10);
    Bank bank;
    bank.addRate("CHF", "USD", 2);
    std::shared_ptr<Sum> sum = std::make_shared<Sum>(fiveDollar, tenFrancs);
    auto sumNew = sum->multipliedBy(2);
    Money result = bank.reduce(*sumNew, "USD");
    EXPECT_EQ(Money::dollar(20)->amount(), result.amount());
}
